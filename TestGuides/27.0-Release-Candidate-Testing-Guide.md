...
To make sure that you have successfully connected to the seed node using v2 Transport, you
can view logs related to `v2` and `peer=0` with the following:

```bash
cat $DATA_DIR/signet/debug.log | grep "v2\|peer=0"
```

Check the logs for messages about v2 or `peer=0` to make sure that our `addrfetch`
connection to the seednode was successful:
