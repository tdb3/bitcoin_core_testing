...
v3 transactions are still treated as non-standard, but we are able to enable it on regtest. Let's do that:
```bash
bcli stop
...
```
Good. Let's see if we are able to RBF the child (sibling eviction). First, lets submit the transactions to our mempool:
```bash
...
```
Create a competing sibling paying higher fees.
```bash
...
```
Submit the new child together with the parent.
```bash
...
840fe6ed69d9239240cbc87c00199d807ba9f99515d9e3155b8aff5fd53277f7
```
Mine 6 blocks so that all transactions are fully confirmed. If we do any less than this, then Coingrinder won't be used. Coin selection also takes number of confirmations into account, so we need all UTXOs in our test wallet to have at least 6 confirmations.  
```bash
bcli -rpcwallet=coinbasewallet generatetoaddress 6 $(bcli -rpcwallet=coinbasewallet getnewaddress)
```
You can use this command to confirm that there is an unspent transaction worth 0.5 tBTC available in the test wallet. Make sure to have `jq` installed.
```bash
...
509a04f1940b30b0d8da0faa743cf41b513dbbe09f9ee6d6f1cb0e506d3117f3
```
To check that only the 0.5 UTXO was used in the transaction, run
```bash
bcli getrawtransaction 509a04f1940b30b0d8da0faa743cf41b513dbbe09f9ee6d6f1cb0e506d3117f3 true
```
using the hash you got from the test transaction. The output should look something like this:
```bash
{
  "txid": "509a04f1940b30b0d8da0faa743cf41b513dbbe09f9ee6d6f1cb0e506d3117f3",
  "hash": "2c943822dfa1e6f2d599d1bdad1a096a4ebcf8bd9a5a2b8c579a9df778ca56c3",
  "version": 2,
  "size": 222,
  "vsize": 141,
  "weight": 561,
  ...
  "vin": [
    {
      "txid": "840fe6ed69d9239240cbc87c00199d807ba9f99515d9e3155b8aff5fd53277f7",
      "vout": 0,
      "scriptSig": {
        "asm": "",
        "hex": ""
      ...
      "txinwitness": [
        "304402201b01ce9ac3cb99a8f47ed1948ae22de7ae2d5c00ed732eb2d7cc81ad6d1c6fb90220714ec6f3eb1ea1185153bf75d665649c14fe72c722699d6ad766596bfc8566c201",
        "036e7f45a38ef739651a189e37140d4607e55b4f2a93081bca99d6d7ea0da8d63e"
      ],
      "sequence": 4294967293
    }
  ],
  "vout": [
    {
      ...
      "n": 0,
      "scriptPubKey": {
        "asm": "0 0d73a5abc7cb3c5874a085132380b8b0ae950120",
        "desc": "addr(bcrt1qp4e6t278ev79sa9qs5fj8q9ckzhf2qfqmf0fyy)#7rj6uzwv",
        "hex": "00140d73a5abc7cb3c5874a085132380b8b0ae950120",
        "address": "bcrt1qp4e6t278ev79sa9qs5fj8q9ckzhf2qfqmf0fyy",
    ...
    },
    {
      ...
      "n": 1,
      "scriptPubKey": {
        "asm": "0 f2e8547efdbbc96fe24aea492101055147ba66fc",
        "desc": "addr(bcrt1q7t59glhah0yklcj2afyjzqg929rm5ehup3vrac)#7ryftsln",
        "hex": "0014f2e8547efdbbc96fe24aea492101055147ba66fc",
        "address": "bcrt1q7t59glhah0yklcj2afyjzqg929rm5ehup3vrac",
        "type": "witness_v0_keyhash"
      }
    }
  ],
  "hex": ""
}
```
Notice in the `vin` section that there is only one input used and that the `txid` in that section (`840fe6ed69d9239240cbc87c00199d807ba9f99515d9e3155b8aff5fd53277f7` in this specific case) is the same hash as the hash we saved from the step above. If we run the `getrawtransaction` command with the saved hash, we'll see that its output value is 0.5. This means that the 0.5 UTXO was the only one selected and that the weight of the inputs was minimized for the test transaction, confirming that the CoinSelector algorithm was used. 

If we were to run these same steps but set the test transaction fee rate to 30 sats/vb or less, we'd notice that a lot more inputs are selected from the test wallet to send 0.47 tBTC. The raw transaction will be a lot larger (therefore paying a lot higher of a transaction fee) than the one created above when CoinSelector was activated in the high fee environment (>30 sats/vb).
