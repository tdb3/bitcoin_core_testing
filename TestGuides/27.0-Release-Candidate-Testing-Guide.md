...
`migratewallet` is [no longer experimental](https://github.com/bitcoin/bitcoin/pull/28037)
in Bitcoin Core v27. This is part of the long road to deprecating legacy wallets and BDB
in Bitcoin Core. At some point, maybe in v29.0, support for Legacy Wallets and BDB may be
[completely removed](https://github.com/bitcoin/bitcoin/pull/28710) from Bitcoin Core, and
the only path forward for legacy wallet users will be to use the `migratewallet` RPC or
to migrate their wallets using the Bitcoin Core GUI. More information about the
timeline for deprecating BDB and Legacy Wallets can be found [in issue
#20160](https://github.com/bitcoin/bitcoin/issues/20160).

### Preparing a Legacy wallet

First, let's start up a node on the regtest network, and with the
`deprecatedrpc=create_bdb` flag set, so that Bitcoin Core doesn't complain when we try to
...
Let's also create a secondary 'helper' wallet and alias that will be useful to us later on.

```bash
cli -named createwallet wallet_name=helper_wallet descriptors=false
alias helper="$(echo $BINARY_PATH)/bitcoin-cli -datadir=$DATA_DIR -rpcwallet=helper_wallet"
```
...
```bash
wallet send -named outputs="{\"$non_HD_address\": 1}"
```
...
### Solvable scripts
**FIXME: I have not checked this section -dg**
...
And verify that our descriptor's and utxo's have been restored:
...
By now, you have a better grasp of what goes into the migration process. Remember, this is
meant to get you started on testing.
