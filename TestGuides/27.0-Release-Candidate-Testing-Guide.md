...
Create the configuration for the v27 and v25 nodes being used:
```bash
echo "regtest=1" > $DATA_DIR_27/bitcoin.conf
```
...
```json5
{
  "txid": "fd1173c201f13352e91b1318e41e2209652287647f2b4946e1a0b7cd03bb1e05",
  "complete": true
}
```
...
> ☝ Run the following commands one at a time and inspect the output, pay attention to the
> values of `ismine`, `solvable`, `iswatchonly`, and `labels`.
...
Do the same for p2sh_segwit, bech32, non_HD_address, watch_address, multisig_address. Pay
attention to the ismine, solvable, iswatchonly and labels fields.

Run the migration and wait. If you are not running bitcoind as a daemon you can also see
the related logging in your console.
...
Once the test is successful, don't forget to [stop your node and clean the
datadir](#5-reset-testing-environment)).
